...
def main(args):
    ...
    sampler = dgl.dataloading.NeighborSampler([5, 10], prob='p')
    ...
    val_sampler = dgl.dataloading.NeighborSampler([5, 10], prob='p')
    ...
    print()
    t1 = time.time()
    print("Total time: : {:.4f}s | Batch time: : {:.4f}s".format(np.sum(bt), np.mean(bt)))
